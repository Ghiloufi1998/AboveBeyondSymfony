...
public class filter_profanity
{
    ...
    private $profanity = array(
        ...
        'vagina',
        'whore','wank','wanker','whoar','terrorist'
    );
    ...
    public function verif2($filter_line, $replace_char='*')
    {
        $forverif=$filter_line;
        foreach($this->profanity as $word)
        {
            $regex = '/(\b|[ \t])';
            $regex_parts = array();
        ...
        if (strcmp($filter_line, $forverif) !== 0) {
            return true;
        }
        return false;
